...
import com.facturas.app.service.RecienNacidoService;
...
public class RecienNacidoController {
    private static final Logger log = LoggerFactory.getLogger(RecienNacidoController.class);

    @Autowired
    private RecienNacidoService service;

    @GetMapping()
    public ResponseEntity<ResponseDTO<List<RecienNacidoDTO>>> getAll() {
        log.info("REST - To get all");
        return ResponseEntity.ok(new ResponseDTO<>(0, service.getAll(), "All Elements", "OK"));
    }

    @PostMapping()
    public ResponseEntity<ResponseDTO<RecienNacidoDTO>> register(@RequestBody RecienNacidoDTO dto) {
        log.info("REST - Request to register : {}", dto);
        return ResponseEntity.ok(new ResponseDTO<>(0, service.register(dto), "Created Element", "OK"));
    }

    @PutMapping()
    public ResponseEntity<ResponseDTO<RecienNacidoDTO>> update(@RequestBody RecienNacidoDTO dto) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ResponseDTO<RecienNacidoDTO>> getById(@PathVariable("id") Integer id) {
