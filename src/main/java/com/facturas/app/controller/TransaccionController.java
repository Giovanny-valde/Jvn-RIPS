...
public class TransaccionController {
    ...
    @GetMapping()
    public ResponseEntity<ResponseDTO<List<TransaccionDTO>>> getAll() {
        log.info("REST - To get all");
        return ResponseEntity.ok(new ResponseDTO<>(0, service.getAll(), "All Elements", "OK"));
    }

    @PostMapping()
    public ResponseEntity<ResponseDTO<TransaccionDTO>> register(@RequestBody TransaccionDTO dto) {
        log.info("REST - Request to register : {}", dto);
        return ResponseEntity.ok(new ResponseDTO<>(0, service.register(dto), "Created Element", "OK"));
    }

    @PutMapping()
    public ResponseEntity<ResponseDTO<TransaccionDTO>> update(@RequestBody TransaccionDTO dto) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ResponseDTO<TransaccionDTO>> getById(@PathVariable("id") Integer id) {
