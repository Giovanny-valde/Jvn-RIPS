...
import com.facturas.app.service.ConsultaService;
...
public class ConsultaController {
    ...
    @GetMapping()
    public ResponseEntity<ResponseDTO<List<ConsultaDTO>>> getAll() {
        log.info("REST - To get all");
        return ResponseEntity.ok(new ResponseDTO<>(0, service.getAll(), "All Elements", "OK"));
    }

    @PostMapping()
    public ResponseEntity<ResponseDTO<ConsultaDTO>> register(@RequestBody ConsultaDTO dto) {
        log.info("REST - Request to register : {}", dto);
        return ResponseEntity.ok(new ResponseDTO<>(0, service.register(dto), "Created Element", "OK"));
    }

    @PutMapping()
    public ResponseEntity<ResponseDTO<ConsultaDTO>> update(@RequestBody ConsultaDTO dto) {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ResponseDTO<ConsultaDTO>> getById(@PathVariable("id") Integer id) {
